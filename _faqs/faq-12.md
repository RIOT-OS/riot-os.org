...
RIOT runs on many different CPU architectures by many different vendors, from [a tiny 8-bit MCU with 2 KiB RAM and 32 KiB flash][riot-arduino-nano] all the way up to high performance [ARM Cortex M][riot-same54-xpro] and [RISC-V MCUs][riot-esp32c3-devkit].
...
This allows developers to pick from a large collection the features they need (and that can fit the MCU).
...
Members in the community mostly belong to one (or more) of the following groups: academia, industry, and the makers/hobbyists.

RIOT is distributed under a copy-left license and strongly prefers free and open source components over proprietary code and binary blobs.
...
Most MCU vendors provide an SDK that is tied and only supports MCU of that specific vendor.
...
The RIOT community can only start working on drivers once the MCUs are generally available and more exotic features that are only relevant on niche use cases may never be supported in favor of a cleaner and more portable API in RIOT.
...
Arduino however goes a step further and sacrifices advanced features such as native multi-threading, a full-fledged and mature network stack, or VFS subsystem for simplicity.
...
The Arduino software is released under a copyleft license and hardware support can be extended greatly beyond the official Arduino boards by using 3rd party Arduino cores.
The development of the ecosystem is sponsored and controlled by a single company that owns the registered trademark of Arduino, while RIOT governed by an independent community.

## Zephyr

Zephyr has a larger and more complex code base and a very sophistication configuration system and unique tooling and is backed by a larger community than RIOT.
This results in a larger feature set at the expense of a steeper learning curve.

RIOT leaner code base makes the code easier to follow and results in a lower memory footprint.
In addition RIOT has been ported to 8-bit and 16-bit platforms, allowing its use on MCUs that are too resource constraint to run Zephyr.
...
Its development is governed by a [technical steering committee][zephyr-tsc] elected from (paying) members, whereas RIOT is steered by [self-governed community][riot-community-process]
...
RIOT provides similar simulation features with the [ZEP packet dispatcher][riot-zep-dispatcher], but clearly Coojia is a more tightly integrated and polished experience.
Another aspect where Contiki-NGs focus on research manifests is that Contiki-NG only supports a small number of boards, but has a quite sophisticated network stack.

As a result of the larger and more active community, RIOT packs more features, has a faster development pace, supports more MCUs families, and has been ported to far more boards.
...
Contiki-NG's development is steered by a small group of (mostly) computer science researchers and distributed under a permissive license.
